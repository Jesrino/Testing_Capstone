...
function loadPatients() {
    ...
    // Use a relative path so the browser sends the current session cookie (avoids cross-origin issues when base_url differs)
    const url = '../api/patients.php?action=get_patients' + (groupBy ? '&group_by=' + groupBy : '');
        ...
        .then(data => {
            ...
            if (data.patients && Object.keys(data.patients).length > 0) {
                ...
                if (groupBy === 'month') {
                    ...
                    allPatients = Array.isArray(data.patients) ? data.patients : Object.values(data.patients);
                }
...
function generatePatientRow(patient) {
    ...
    if (patient.type === 'registered') {
        ...
        // For walk-in patients, use name and phone to identify them
    ...
    return `
        <tr>
            <td><span class="patient-name">${escapeHtml(patient.name)}</span></td>
            <td><span class="patient-type ${patient.type}">${patient.type.charAt(0).toUpperCase() + patient.type.slice(1)}</span></td>
